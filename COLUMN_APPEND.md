...
```r
library(fst)
append_columns_fst(data.frame(new_library = counts), "chr1_chunk_001.fst")
```
...
durable; retain source libraries/backups. See `?append_columns_fst` for details.

For ORFik, preserve each existing chromosome/chunk row mapping and use
`append_columns_fst(new_library_columns_for_that_chunk, chunk_path)` in place of
