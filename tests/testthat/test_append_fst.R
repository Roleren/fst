...
test_that("repeated appends preserve original bytes and allow reordered partial reads", {
  ...
  for (compression in c(0, 50, 100)) {
    ...
    expect_identical(append_fst(y, path, compress = compression), y)
    ...
    append_fst(z, path, compress = compression)
...
test_that("all supported R column types and annotations survive append", {
  ...
  for (compression in c(0, 50, 100)) {
    write_fst(x, path)
    append_fst(y, path, compress = compression, uniform_encoding = TRUE)
...
test_that("empty rows and data.table keys work", {
  ...
  append_fst(data.frame(b = character(), c = double()), path)
  ...
  for (key in list("id", c("id", "value"))) {
    ...
    write_fst(x, path)
    append_fst(data.frame(lib = 11:20), path)
...
test_that("invalid requests leave the original table readable", {
  ...
  write_fst(x, path)
  before <- append_bytes(path)
  expect_error(append_fst(data.frame(b = 1:2), path), "same number")
  expect_error(append_fst(x, path), "names")
  bad <- data.frame(b = 1:3, c = 1:3)
  names(bad) <- c("b", "b")
  expect_error(append_fst(bad, path), "names")
  names(bad) <- c("", NA)
  expect_error(append_fst(bad, path), "names")
  expect_error(append_fst(data.frame(), path), "at least one")
  for (value in list(NA, numeric(), c(1, 2), -1, 101, Inf, "50")) {
    expect_error(append_fst(data.frame(b = 1:3), path, compress = value), "compress")
  }
  for (value in list(NA, logical(), c(TRUE, FALSE), 1)) {
    expect_error(append_fst(data.frame(b = 1:3), path, uniform_encoding = value), "uniform_encoding")
  }
  expect_identical(append_bytes(path), before)
  expect_error(append_fst(data.frame(b = 1:3, bad = complex(3)), path), "type|Type")
  expect_identical(read_fst(path), x)
  append_fst(data.frame(c = 4:6), path)
...
test_that("names preserve Unicode and compare across encodings", {
  ...
  expect_error(append_fst(same_name, path), "names")
  new <- data.frame(a = c("a", "b", NA))
  names(new) <- enc2utf8("新文庫")
  append_fst(new, path, uniform_encoding = FALSE)
...
test_that("zero-column files accept empty columns and fst objects can read appends", {
  ...
  append_fst(data.frame(a = integer()), path)
